...
def add(*decomposition_functions):
    """
    Combines the results of multiple decomposition functions into a single 
    quotient graph by merging the nodes and edges produced by each function.

    Parameters:
    - decomposition_functions: A variable number of functions to be applied 
      independently to a `QuotientGraph`. Each function should take a 
      `QuotientGraph` as input and return a modified `QuotientGraph` 
      containing additional or updated nodes and edges.

    Returns:
    - A single function (`composed`) that takes a `QuotientGraph` as input, 
      applies all the decomposition functions independently, and merges 
      their results into a single `QuotientGraph`.
    """
    def composed(quotient_graph: 'QuotientGraph'):
        """
        Applies the decomposition functions to the input quotient graph 
        and combines their results into a single quotient graph.

        Parameters:
        - quotient_graph: The input `QuotientGraph` to which the decomposition 
          functions are applied.

        Returns:
        - A new `QuotientGraph` that combines the results of all decomposition functions.
        """
        return sum((func(quotient_graph) for func in decomposition_functions), QuotientGraph())

    # Return the composed function that combines results from decomposition functions
    return composed

#--------------------------------------------------------------------------------
def compose(*decomposition_functions):
    """
    Composes multiple functions into a single function that applies them sequentially
    in reverse order. This is useful for chaining transformations or operations
    on a `QuotientGraph` object.

    Parameters:
    - decomposition_functions: A variable number of functions to be composed. 
      Each function should take a `QuotientGraph` as input and return a modified 
      `QuotientGraph` (or equivalent) as output.

    Returns:
    - A single function (`composed`) that takes a `QuotientGraph` as input, 
      applies the given functions in reverse order, and returns the final result.
    """
    def composed(quotient_graph: 'QuotientGraph'):
        """
        Applies the composed functions sequentially to the input `quotient_graph`.

        Parameters:
        - quotient_graph: The input object (of type `QuotientGraph`) to which 
          the functions are applied.

        Returns:
        - The modified `QuotientGraph` after applying all functions in reverse order.
        """
        for func in reversed(decomposition_functions):
            # Apply each function to the quotient_graph, starting with the last one
            quotient_graph = func(quotient_graph)
        return quotient_graph  # Return the fully transformed quotient_graph
...
def identity(
    ...
    out_quotient_graph = QuotientGraph(quotient_graph)
...
def node(
    ...
    out_quotient_graph = QuotientGraph().partial_copy(quotient_graph)

    for subgraph in quotient_graph.subgraphs():
        components = [[node] for node in subgraph.nodes()]
        for component in components:
            out_quotient_graph.add_quotient_node(nodes=component)
    ...
    for subgraph in quotient_graph.subgraphs():
        ...
        for r in range(min(radius), max(radius) + 1):
            ...
            for source in subgraph.nodes():
                ...
                out_quotient_graph.add_quotient_node(nodes=reachable_nodes)
...
def cycle(
    ...
    for subgraph in quotient_graph.subgraphs():
        ...
        for cycle_component in cycle_components:
            out_quotient_graph.add_quotient_node(nodes=cycle_component)
...
def tree(
    ...
    for subgraph in quotient_graph.subgraphs():
        ...
        for non_cycle_component in non_cycle_components:
            out_quotient_graph.add_quotient_node(nodes=non_cycle_component)
...
def complement(
    ...
    for subgraph in quotient_graph.subgraphs():
        ...
        negative_component = set(quotient_graph.graph.nodes()).difference(set(component))
        out_quotient_graph.add_quotient_node(nodes=negative_component)
...
def merge(
    ...
    if use_edges:
        ...
        out_quotient_graph.add_quotient_node(edges=component)
...
def combination(
    ...
    components = combination_decomposition_function(quotient_graph.subgraphs(), quotient_graph.graph, number_of_elements=number_of_elements, distance=distance)
    for component in components:
        out_quotient_graph.add_quotient_node(nodes=component)
...
def filter_by_number_of_connected_components(
    ...
    for subgraph in quotient_graph.subgraphs():
        ...
        if min(number_of_components)<=len(cc)<=max(number_of_components):
            out_quotient_graph.add_quotient_node(nodes=subgraph.nodes())
...
def binary_combination(
    ...
    out_quotient_graph = QuotientGraph().partial_copy(first_quotient_graph)
    components = binary_combination_decomposition_function(first_quotient_graph.subgraphs(), second_quotient_graph.subgraphs(), first_quotient_graph.graph, distance=distance)
    for component in components:
        out_quotient_graph.add_quotient_node(nodes=component)
...
def intersection_edges(
    ...
    out_quotient_graph = QuotientGraph().partial_copy(quotient_graph)
    for u in quotient_graph.quotient_nodes():
        out_quotient_graph.add_quotient_node(nodes=quotient_graph.subgraph_nodes(u))

    for u in quotient_graph.quotient_nodes():
        nodes_u = quotient_graph.subgraph_nodes(u)
        for v in quotient_graph.quotient_nodes():
            if u != v:
                nodes_v = quotient_graph.subgraph_nodes(v)
                if len(set(nodes_u).intersection(set(nodes_v)))>=size_threshold:
                    out_quotient_graph.add_quotient_edge(u,v)
...
def unlabel(
    ...
    out_quotient_graph = QuotientGraph(quotient_graph)
    nx.set_node_attributes(out_quotient_graph.graph,label,'label')
    nx.set_edge_attributes(out_quotient_graph.graph,label,'label')
    out_quotient_graph.set_quotient_node_and_edge_labels()
...
#====================================================================================================
@curry
def number_of_quotient_nodes(
    quotient_graph: 'QuotientGraph',
    param=None
) -> int:
    return quotient_graph.quotient_graph.number_of_nodes()

def number_of_quotient_edges(
    quotient_graph: 'QuotientGraph',
    param=None
) -> int:
    return quotient_graph.quotient_graph.number_of_edges()

def quantile_number_of_subgraph_nodes(
    quotient_graph: 'QuotientGraph',
    q=0.5
) -> int:
    return np.quantile([subgraph.number_of_nodes() for subgraph in quotient_graph.subgraphs()], q)

def quantile_number_of_subgraph_edges(
    quotient_graph: 'QuotientGraph',
    q=0.5
) -> int:
    return np.quantile([subgraph.number_of_edges() for subgraph in quotient_graph.subgraphs()], q)

def max_number_of_subgraph_nodes(
    quotient_graph: 'QuotientGraph',
    param=None
) -> int:
    return max([subgraph.number_of_nodes() for subgraph in quotient_graph.subgraphs()])

def min_number_of_subgraph_nodes(
    quotient_graph: 'QuotientGraph',
    param=None
) -> int:
    return min([subgraph.number_of_nodes() for subgraph in quotient_graph.subgraphs()])

def max_number_of_subgraph_edges(
    quotient_graph: 'QuotientGraph',
    param=None
) -> int:
    return max([subgraph.number_of_edges() for subgraph in quotient_graph.subgraphs()])

def min_number_of_subgraph_edges(
    quotient_graph: 'QuotientGraph',
    param=None
) -> int:
    return min([subgraph.number_of_edges() for subgraph in quotient_graph.subgraphs()])