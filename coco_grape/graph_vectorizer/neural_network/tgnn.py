...
class GraphEncoder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, graphs: List[nx.Graph]) -> List[Data]:
        ...
        data_list = []
        for idx, G in enumerate(graphs):
            # Relabel nodes to have contiguous integer labels starting at 0
            mapping = {node: i for i, node in enumerate(G.nodes())}
            G_relabel = nx.relabel_nodes(G, mapping)

            # Encode nodes
            node_features = []
            for node in G_relabel.nodes(data=True):
            ...
            edge_indices = []
            edge_features = []
            for edge in G_relabel.edges(data=True):
                u, v, attr = edge
...
class TransformerConvLayer(nn.Module):
    def __init__(
        ...
        n_heads: int = 4,
        dropout: float = 0.1,
        max_neighbors: int = 10  # Maximum number of neighbors to consider
    ):
        super(TransformerConvLayer, self).__init__()
        self.hidden_dim = hidden_dim
        self.n_heads = n_heads
        self.d_model = hidden_dim * n_heads  # Set d_model to hidden_dim * n_heads

        # Ensure that d_model is divisible by n_heads
        assert self.d_model % self.n_heads == 0, "d_model must be divisible by n_heads."

        # Project node features to d_model
        self.input_proj = nn.Linear(input_dim, self.d_model)
        self.marker_proj = nn.Linear(1, self.d_model)  # For marker bits

        # Define the TransformerEncoder with d_model = hidden_dim * n_heads
        self.transformer_encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=self.d_model, nhead=self.n_heads, dropout=dropout),
            num_layers=1
        )
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, batch: torch.Tensor) -> torch.Tensor:
        """
        Forward pass for the TransformerConvLayer.

        Parameters:
        - x: Node feature matrix [num_nodes, input_dim]
        - edge_index: Edge indices [2, num_edges]
        - batch: Batch vector [num_nodes] assigning each node to a graph
        ...
        # Add self-loops to include the node itself in its neighborhood
        edge_index_with_self, _ = torch_geometric.utils.add_self_loops(edge_index, num_nodes=num_nodes)
        edge_index_with_self = edge_index_with_self[:2]  # Ensure it's [2, E]

        # Debugging: Check edge_index_with_self
        if hasattr(self, 'verbose') and self.training and self.max_neighbors == 10 and self.verbose >= 2:
            print(f"Edge Index Shape: {edge_index_with_self.shape}")
            print(f"Edge Index Contents:\n{edge_index_with_self}")

        # Ensure edge_index_with_self is 2-dimensional
        assert edge_index_with_self.dim() == 2 and edge_index_with_self.size(0) == 2, \
            f"edge_index must be [2, E], got {edge_index_with_self.shape}"

        row, col = edge_index_with_self

        # Check row is 1D tensor of non-negative integers within [0, num_nodes -1]
        assert row.dim() == 1, f"row should be 1D tensor, got {row.dim()}D."
        assert torch.all(row >= 0), "row contains negative indices."
        assert torch.all(row < num_nodes), f"row contains indices >= num_nodes ({num_nodes})."

        # Compute degrees using torch.bincount
        deg = torch.bincount(row, minlength=num_nodes)

        # Initialize neighbor indices and mask
        neighbor_indices = torch.full((num_nodes, self.max_neighbors), -1, dtype=torch.long, device=device)
        mask = torch.zeros((num_nodes, self.max_neighbors), dtype=torch.bool, device=device)

        # Populate neighbor_indices and mask
        for node in range(num_nodes):
            # Squeeze only the second dimension to avoid 0-dim tensor
            start = (row == node).nonzero(as_tuple=False).squeeze(1)
            if deg[node] > 0:
                neighbors = col[start]
                num_neighbors = min(deg[node], self.max_neighbors)
                # Ensure neighbors has at least num_neighbors elements
                if neighbors.size(0) < num_neighbors:
                    neighbors = torch.cat([
                        neighbors,
                        torch.zeros(num_neighbors - neighbors.size(0), dtype=neighbors.dtype, device=device)
                    ])
                neighbor_indices[node, :num_neighbors] = neighbors[:num_neighbors]
                mask[node, :num_neighbors] = 1

        # Gather neighbor features, replacing invalid indices with zeros
        # Note: neighbor_indices may contain -1, so clamp to 0 and mask later
        neighbor_features = torch.where(
            neighbor_indices.unsqueeze(-1) != -1,
            x[neighbor_indices.clamp(min=0)],
            torch.zeros(x.size(1), device=device).unsqueeze(0).expand(num_nodes, self.max_neighbors, -1)
        )  # [num_nodes, max_neighbors, input_dim]

        # Create marker bits: 1 for the node itself, 0 for neighbors
        # Assuming the first neighbor is always the node itself due to self-loops
        marker_bits = torch.zeros((num_nodes, self.max_neighbors, 1), device=device)
        marker_bits[:, 0, 0] = 1.0  # First token is the node itself

        # Concatenate marker bits to features
        tokens = torch.cat([neighbor_features, marker_bits], dim=-1)  # [num_nodes, max_neighbors, input_dim +1]

        # Project features
        tokens = self.input_proj(tokens[:, :, :-1]) + self.marker_proj(tokens[:, :, -1:].float())  # [num_nodes, max_neighbors, d_model]

        # Reshape for transformer: [max_neighbors, num_nodes, d_model]
        tokens = tokens.permute(1, 0, 2)  # [max_neighbors, num_nodes, d_model]

        # Pass through transformer
        transformed = self.transformer_encoder(tokens)  # [max_neighbors, num_nodes, d_model]

        # Reshape back to [num_nodes, max_neighbors, d_model]
        transformed = transformed.permute(1, 0, 2).contiguous()  # [num_nodes, max_neighbors, d_model]

        # Apply mask to ignore padding tokens
        mask = mask.unsqueeze(-1)  # [num_nodes, max_neighbors, 1]
        transformed = transformed * mask  # Zero out padded positions

        # Aggregate transformed features (e.g., mean over neighbors)
        node_features = transformed.sum(dim=1) / mask.sum(dim=1).clamp(min=1)  # [num_nodes, d_model]

        # Project back to hidden_dim
        node_features = self.output_proj(node_features)  # [num_nodes, hidden_dim]

        return node_features


class GTNEncoder(LightningModule):
    def __init__(
        self,
        input_dim: int,
        edge_dim: int,  # Not used in TransformerConvLayer, but kept for compatibility
        hidden_dim: int,
        output_dim: int,
        num_layers: int,
        ...
        Uses TransformerEncoderLayer for message passing.

        Parameters:
        - input_dim: Dimensionality of input node features
        - edge_dim: Dimensionality of edge features (not used here)
        - hidden_dim: Dimensionality of hidden layers
        - output_dim: Dimensionality of the final embedding
        ...
        - aggr: Aggregation method (not used)
        - warmup_start_lr: Initial learning rate for warmup
        - warmup_end_lr: Target learning rate after warmup
        - warmup_epochs: Number of epochs for warmup
        - verbose: Verbosity level
        - n_heads: Number of attention heads in TransformerConvLayer
        - max_neighbors: Maximum number of neighbors to consider in TransformerConvLayer
        """
        super(GTNEncoder, self).__init__()
        self.save_hyperparameters()

        # Assign hyperparameters as class attributes
        ...
        # Define TransformerConv layers with dynamic number of hidden layers
        # Input layer
        self.convs.append(TransformerConvLayer(
            input_dim=input_dim,
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        for conv, batch_norm, dropout, leaky_relu, residual in zip(
            ...
            x = conv(x, edge_index, batch)  # Transformer-based message passing
...
class GTTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(
        ...
        """
        Fits the GTTransformer model on the provided graphs.
        ...
        if edge_dim == 0:
            if self.verbose >=1:
                print("Edge attributes are required for TransformerConvLayer. Ensure that edges have 'label' and optionally 'vec' attributes.")
            raise ValueError("Edge attributes are required for TransformerConvLayer. Ensure that edges have 'label' and optionally 'vec' attributes.")

        # Determine max_neighbors by finding the maximum degree across all graphs
        max_neighbors = 0
        for idx, G in enumerate(graphs):
            degrees = [degree for node, degree in G.degree()]
            graph_max = max(degrees) if degrees else 0
            if graph_max > max_neighbors:
                max_neighbors = graph_max
            if self.verbose >=2:
                print(f"Graph {idx}: Max neighbors = {graph_max}")

        if self.verbose >=1:
            print(f"Determined max_neighbors for TransformerConvLayer: {max_neighbors}")

        # Initialize the model with warmup parameters and max_neighbors
        self.model = GTNEncoder(
            input_dim=input_dim,
            edge_dim=edge_dim,
            ...
            max_neighbors=max_neighbors               # Pass the determined max_neighbors
        )
        ...
        if os.path.exists(checkpoint_path):
            ...
            self.model = GTNEncoder.load_from_checkpoint(checkpoint_path, strict=False)
            self.model.to(self.device)
            self.model.eval()
        else:
            if self.verbose >=1:
