...
class CTGNNEncoder(pl.LightningModule):
    def __init__(
        self,
        input_dim: int,
        edge_dim: int,
        ...
        [Original Docstring]
        """
        super(CTGNNEncoder, self).__init__()
        ...
        self.layer_norms = nn.ModuleList()  # Changed from batch_norms to layer_norms
        ...
        self.layer_norms.append(nn.LayerNorm(hidden_dim))  # Replaced BatchNorm1d with LayerNorm
        ...
        # Transformer Encoder after NNConv layers
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        for conv, layer_norm, dropout, leaky_relu, residual in zip(
            ...
            if x.ndim > 1 and x.shape[0] > 1:  # Ensure layer normalization is applied appropriately
                x = layer_norm(x)
            x = leaky_relu(x)
            x = dropout(x)
        ...
        # Prepare for Transformer: reshape x to [num_nodes, batch_size, hidden_dim]
        # PyTorch Transformer expects [sequence_length, batch_size, embed_dim]
        # However, in PyG, x is [num_nodes_in_batch, hidden_dim] and batch is [num_nodes_in_batch]
        # We'll need to group node embeddings by graph, pad them, and create a mask
        # For simplicity, we'll process the entire batch as one sequence without considering graph boundaries

        # Optional: You can handle per-graph Transformer by iterating over each graph in the batch
        # However, this is less efficient. Here, we process all node embeddings together.

        # Reshape x to [num_nodes, 1, hidden_dim] to simulate batch_size=1
        x_trans = x.unsqueeze(1)  # [num_nodes, 1, hidden_dim]
        transformer_output = self.transformer_encoder(x_trans)  # [num_nodes, 1, hidden_dim]
        x = transformer_output.squeeze(1)  # [num_nodes, hidden_dim]
    ...
    def get_node_embeddings(self, x, edge_index, edge_attr, batch):
        ...
        return x  # Return node embeddings before pooling
...
class CTGNNTransformer(BaseEstimator, TransformerMixin):
    def __init__(
        self,
        hidden_dim: int = 64,
        output_dim: int = 128,
        ...
        """
        CTGNNTransformer: A Scikit-Learn Compatible Transformer for Graph Neural Networks with Transformer Integration.

        The `CTGNNTransformer` class provides a seamless interface for training and utilizing a Graph Neural Network (GNN)
        enhanced with Transformer encoder layers. It leverages PyTorch Geometric for graph data handling and PyTorch Lightning
        for streamlined training workflows. This transformer is compatible with Scikit-Learn pipelines, enabling easy integration
        into machine learning workflows.

        ### Features:
        - **Graph Encoding**: Converts NetworkX graphs into PyTorch Geometric `Data` objects with encoded node and edge features.
        - **Customizable GNN Architecture**: Supports dynamic hidden layers, dropout, LeakyReLU activations, batch normalization,
          residual connections, and a Transformer encoder.
        - **Task Flexibility**: Handles both classification and regression tasks.
        - **Training Utilities**: Includes early stopping, model checkpointing, learning rate warmup, and detailed loss recording.
        - **Visualization**: Optionally plots training and validation metrics for performance monitoring.
        - **Parallel Processing**: Utilizes multiple workers for efficient data loading.
        ...
        Args:
            ...
            output_dim (int, optional): 
                Dimensionality of the output embeddings produced by the GNN.  
                **Default**: `128`.
            
            edge_hidden_dim (int, optional): 
                Dimensionality of the hidden layers for edge feature transformations.  
                **Default**: `64`.
            ...
            num_workers (Optional[int], optional): 
                Number of subprocesses to use for data loading. If `None`, uses the number of CPU cores available.  
                **Default**: `None`.
            
            plot_training (bool, optional): 
                If `True`, plots training and validation loss and performance metrics after training.  
                **Default**: `False`.
            ...
            warmup_end_lr (float, optional): 
                Target learning rate after completing the warmup phase.  
                **Default**: `1e-3`.
            
            warmup_epochs (int, optional): 
                Number of epochs over which to perform learning rate warmup.  
                **Default**: `50`.
            
            device (Optional[str], optional): 
                Device to run the model on, either `'cuda'` for GPU or `'cpu'`. If `None`, automatically selects based on availability.  
                **Default**: `None`.
            
            aggr (str, optional): 
                Aggregation method for NNConv layers (e.g., `'sum'`, `'mean'`, `'max'`).  
                **Default**: `'sum'`.
            ...
            transformer_hidden_dim (int, optional): 
                Hidden dimensionality for the Transformer encoder layers.  
                **Default**: `128`.
            ...
            transformer_num_heads (int, optional): 
                Number of attention heads in each Transformer encoder layer.  
                **Default**: `8`.
            
            verbose (int, optional): 
                Verbosity level for logging and debug information:
                    - `0`: No output.
        ...
        ### Attributes:
            graph_encoder (GraphEncoder):
                An instance of `GraphEncoder` responsible for encoding NetworkX graphs into PyTorch Geometric `Data` objects.
            
            model (CTGNNEncoder):
                The underlying PyTorch Lightning model encapsulating the GNN architecture with Transformer integration.
            
            is_fitted_ (bool):
                Indicates whether the transformer has been fitted. Set to `True` after successful training.
            
            task_type (str):
                Specifies the type of task: `'classification'` or `'regression'`.
            
            num_classes (Optional[int]):
                Number of classes for classification tasks. `None` for regression tasks.
            
            loss_recorder (LossRecorder):
                PyTorch Lightning callback instance for recording training and validation losses and metrics.

        ### Methods:
            fit(graphs, y, checkpoint_dir):
                Trains the GNN model on the provided graph data and labels.

            transform(graphs):
                Transforms input graphs into their corresponding vector embeddings using the trained GNN model.

            fit_transform(graphs, y):
                Combines the `fit` and `transform` methods for convenience, fitting the model and returning embeddings.

            _plot_metrics(window, use_pandas):
                (Private Method) Plots training and validation loss and performance metrics with optional running mean smoothing.

        ### Usage Example:

        ```python
        import networkx as nx
        from CTGNNTransformer import CTGNNTransformer

        # Sample data: list of NetworkX graphs and corresponding labels
        graphs = [nx.Graph() for _ in range(100)]
        labels = [0, 1] * 50  # Binary classification labels

        # Initialize the transformer with custom Transformer parameters
        ctgnn = CTGNNTransformer(
            hidden_dim=128,
            output_dim=256,
            edge_hidden_dim=64,
            num_layers=3,
        ...
        ctgnn.fit(graphs=graphs, y=labels)

        # Transform new graphs into embeddings
        new_graphs = [nx.Graph() for _ in range(10)]
        embeddings = ctgnn.transform(new_graphs)

        print(embeddings.shape)  # Expected shape: (10, output_dim)
        ```

        ### Notes:
        - **Graph Requirements**: Each NetworkX graph must have node attributes `'label'` and optionally `'vec'`. Similarly, edges must have a `'label'` attribute and optionally a `'vec'` attribute. The `GraphEncoder` handles encoding these attributes into one-hot vectors concatenated with continuous vectors if provided.
        
        - **Device Management**: The transformer automatically selects `'cuda'` if available unless a specific device is provided. Ensure that the specified device has sufficient memory, especially when using large Transformer models.
        
        - **Transformer Integration**: The Transformer encoder processes node embeddings post-GNN layers, allowing for enhanced contextual representation. Adjust `transformer_num_layers` and `transformer_num_heads` based on the complexity of your data and available computational resources.
        
        - **Early Stopping and Checkpointing**: The transformer employs early stopping based on validation loss to prevent overfitting. Best model checkpoints are saved automatically, and the best model is loaded after training.
        
        - **Parallel Data Loading**: Set `num_workers` to leverage multiple CPU cores for faster data loading, especially beneficial for large datasets.
        
        - **Visualization**: If `plot_training` is enabled, training and validation loss and metrics are visualized post-training for performance assessment.
        ...
        - **Scalability**: Increasing the number of Transformer layers or attention heads can significantly impact training time and memory usage. Optimize these parameters based on your hardware capabilities.
        
        - **Data Preprocessing**: Properly preprocess your graph data to include necessary node and edge attributes. Inconsistent attribute dimensions across graphs will raise errors.
        
        - **Task Specification**: Clearly specify whether your task is classification or regression by providing appropriate labels. The transformer automatically adjusts its output layers and loss functions based on the task type.
        
        - **Reproducibility**: Set the `random_state` parameter to ensure reproducible training results across different runs.

        By integrating the `CTGNNTransformer` into your machine learning pipeline, you can efficiently train and utilize advanced graph-based models with Transformer enhancements for a variety of tasks.
        """
        ...
        self.aggr = aggr
        self.dropout_prob = dropout_prob
        self.transformer_hidden_dim = transformer_hidden_dim  # Store Transformer hidden dimension
        self.transformer_num_layers = transformer_num_layers  # Store Transformer number of layers
        self.transformer_num_heads = transformer_num_heads    # Store Transformer number of heads
        self.verbose = verbose
    ...
    def fit(
        ...
        if os.path.exists(checkpoint_path):
            ...
            self.model = CTGNNEncoder.load_from_checkpoint(
                checkpoint_path, 
                strict=False,
    ...
    def transform(self, graphs: List[nx.Graph]) -> np.ndarray:
        ...
        This method encodes the provided graphs into PyTorch Geometric `Data` objects using the
        `GraphEncoder`, processes them through the trained Graph Neural Network (GNN) model,
        and returns their corresponding embeddings as a NumPy array.

        Parameters
        ----------
        graphs : List[nx.Graph]
            A list of NetworkX graph objects to be transformed into embeddings. Each graph must have:
            - Node attributes:
                - `'label'`: Categorical label for each node.
                - `'vec'` (optional): Continuous vector attributes for each node.
            - Edge attributes:
                - `'label'`: Categorical label for each edge.
                - `'vec'` (optional): Continuous vector attributes for each edge.
            The `GraphEncoder` will handle the encoding of these attributes into numerical formats.

        Returns
        -------
        np.ndarray
            A NumPy array of shape `(num_graphs, output_dim)`, where `num_graphs` is the number of input graphs
            and `output_dim` is the dimensionality of the output embeddings as specified during the
            `CTGNNTransformer` initialization.

        Raises
        ------
        NotFittedError
            If the transformer has not been fitted using the `fit` method prior to calling `transform`.
        ValueError
            If the input graphs are improperly formatted, such as missing required node or edge attributes.

        Notes
        -----
        - **Batch Processing**: Graphs are processed in batches for computational efficiency. The batch size
          is determined by the `batch_size` parameter provided during initialization.
        - **Device Management**: The method utilizes the device (CPU or GPU) specified during initialization.
          Ensure that the device has sufficient memory, especially when processing large graphs or using
          high-dimensional embeddings.
        - **Model Evaluation Mode**: The GNN model is set to evaluation mode (`model.eval()`) to ensure that
          layers like dropout and batch normalization operate in inference mode.
        - **No Gradient Computation**: Embedding generation is performed within a `torch.no_grad()` context to
          disable gradient computations, which reduces memory consumption and speeds up the process.
        - **Verbosity Levels**:
            - `verbose=1`: Displays basic transformation progress.
            - `verbose=2`: Provides detailed batch processing information, including the shape of embeddings per batch.
