...
including node-level regression, offering enhanced flexibility and performance by 
...
class GraphEncoder(BaseEstimator, TransformerMixin):
    ...
    Additionally, it extracts per-node 'output' vectors for regression tasks.
    ...
    def __init__(self, verbose: int = 1):
        ...
        self.node_vec_dim = 0
        self.edge_vec_dim = 0
        self.output_dim = 0  # Dimension of 'output' vectors
    ...
    def fit(self, graphs: List[nx.Graph], y=None):
        ...
        Also determines the dimension of 'output' vectors.
        ...
        for idx, G in enumerate(graphs):
            ...
            for node in G.nodes(data=True):
                if 'output' in node[1]:
                    output_dims.append(len(node[1]['output']))
                else:
                    if self.verbose >= 2:
                        print(f"Graph {idx}: Node {node[0]} missing 'output' attribute.")

        # Ensure all nodes have 'output' vectors
        for idx, G in enumerate(graphs):
            for node in G.nodes(data=True):
                if 'output' not in node[1]:
                    if self.verbose >=1:
                        print(f"Graph {idx}: Node {node[0]} is missing 'output' attribute. Setting to zeros.")
                    # Assign zero vector if 'output' is missing
                    node[1]['output'] = [0.0] * self.output_dim if self.output_dim > 0 else [0.0]
        ...
        # Determine 'output' vector dimension
        if output_dims:
            if len(set(output_dims)) != 1:
                if self.verbose >=2:
                    print("All node 'output' vectors must have the same dimension.")
                raise ValueError("All node 'output' vectors must have the same dimension.")
            self.output_dim = output_dims[0]
        else:
            if self.verbose >=1:
                print("No 'output' vectors found in any nodes. Setting output_dim to 0.")
            self.output_dim = 0  # No 'output' vectors present
        ...
        if self.verbose >= 1:
            ...
            print(f"'output' Vector Dimension: {self.output_dim}")
    ...
    def transform(self, graphs: List[nx.Graph]) -> List[Data]:
        ...
        for idx, G in enumerate(graphs):
            ...
            for node in G.nodes(data=True):
                ...
                output = node[1].get('output') if self.output_dim > 0 else None
                ...
                # Handle 'output' vectors
                if self.output_dim > 0:
                    if output is not None:
                        if len(output) != self.output_dim:
                            if self.verbose >= 2:
                                print(f"Graph {idx}: All node 'output' vectors must have dimension {self.output_dim}.")
                            raise ValueError(f"Graph {idx}: All node 'output' vectors must have dimension {self.output_dim}.")
                        node_outputs.append(output)
                    else:
                        if self.verbose >=1:
                            print(f"Graph {idx}: Node {node[0]} missing 'output' attribute. Setting to zeros.")
                        node_outputs.append([0.0] * self.output_dim)
            ...
            # Assign node-level 'output' vectors if present
            if self.output_dim > 0:
                data.y = torch.tensor(node_outputs, dtype=torch.float)
...
class LossRecorder(pl.Callback):
    ...
    and Mean Squared Errors (MSE) for node-level regression.
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        if self.verbose >= 2:
            ...
            if train_mse is None:
                print(f"Warning: 'train_mse' not found for epoch {self.epoch_count}")
        ...
        if train_mse is not None:
            ...
            if self.verbose >= 1:
                print(f"Warning: 'train_mse' not found for epoch {self.epoch_count}. Skipping MSE recording.")
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if self.verbose >= 2:
            ...
            if val_mse is None:
                print(f"Warning: 'val_mse' not found for epoch {self.epoch_count}")
        ...
        if val_mse is not None:
            ...
            if self.verbose >= 1:
                print(f"Warning: 'val_mse' not found for epoch {self.epoch_count}. Skipping MSE recording.")
...
class CrossAttentionConvolutionTransformerGraphNeuralNetworkEncoder(pl.LightningModule):
    def __init__(
        ...
        output_dim: int,
        num_layers: int,
        task_type: str = 'regression',  # Changed default to 'regression'
        num_classes: Optional[int] = None,  # Not used for regression
        ...
        supporting cross-attention between NNConv layers and Transformer layers for node-level regression tasks.
        """
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        ...
        out = self.fc_out(x)  # [num_nodes, output_dim]
        return out

    def get_node_embeddings(self, x, edge_index, edge_attr, batch):
        """
        Returns the node embeddings after the last transformer layer but before the final regression layer.
        """
        nnconv_embeddings = []
        ...
        transformer_output = x  # Initialize with the last NNConv layer's output
        node_embeddings = transformer_output  # To keep track before final layer
        ...
        return node_embeddings  # Return node embeddings before the final regression layer


    def training_step(self, batch, batch_idx):
        outputs = self.forward(batch.x, batch.edge_index, batch.edge_attr, batch.batch)
        targets = batch.y  # [num_nodes, output_dim]
...
class CrossAttentionConvolutionTransformerGraphNeuralNetworkTransformer(BaseEstimator, TransformerMixin):
    ...
    This version has been modified to support node-level regression without a distinct per-graph target.
    
    Parameters
    ----------
    [Parameters remain largely the same, except for task_type being fixed to 'regression']
    """
    def __init__(
        self,
        hidden_dim: int = 64,
        ...
        """
        CrossAttentionConvolutionTransformerGraphNeuralNetworkTransformer is a 
        Scikit-Learn compatible transformer designed for node-level regression on graph-structured data.
        
        This version removes the distinct y target and instead utilizes per-node 'output' vectors for regression.
        
        Parameters
        ----------
        [Comprehensive docstring updated to reflect node-level regression]
        """
        ...
        self.is_fitted_ = False
        self.output_dim_final = None  # To be set based on GraphEncoder
        self.loss_recorder = LossRecorder(verbose=self.verbose >=2)  # Initialize the LossRecorder callback

    def fit(
        self,
        graphs: List[nx.Graph],
        y: Optional[Union[np.ndarray, List]] = None,  # Ignored
        checkpoint_dir: str = "checkpoints"  # Set default checkpoint directory
    ):
        """
        Fits the GNN model on the provided graphs using node-level 'output' vectors for regression.
        Integrates SWA if enabled.

        Parameters:
        - graphs: List of NetworkX graphs.
        - y: Ignored. Per-node 'output' vectors are used instead.
        ...
        """
        if self.verbose >=1:
            print("Starting fit process for node-level regression.")
        ...
        if self.graph_encoder.output_dim == 0:
            if self.verbose >=1:
                print("No 'output' vectors found in any nodes. Cannot proceed with regression.")
            raise ValueError("No 'output' vectors found in any nodes. Each node must have an 'output' attribute for regression.")

        # Encode graphs
        data_list = self.graph_encoder.transform(graphs)

        # Assign node-level 'output' vectors
        for data in data_list:
            if not hasattr(data, 'y'):
                if self.verbose >=1:
                    print("A graph in data_list is missing 'y' attribute. Setting to zeros.")
                data.y = torch.zeros((data.num_nodes, self.graph_encoder.output_dim), dtype=torch.float)
        ...
        self.output_dim_final = self.graph_encoder.output_dim

        if self.verbose >=1:
            print(f"Training Input Dimension: {input_dim}")
            print(f"Training Edge Dimension: {edge_dim}")
            print(f"Node 'output' Vector Dimension: {self.output_dim_final}")
        ...
        self.model = CrossAttentionConvolutionTransformerGraphNeuralNetworkEncoder(
            ...
            output_dim=self.output_dim_final,
            num_layers=self.num_layers,
            task_type='regression',  # Fixed to 'regression'
        ...
        # Plot training and validation loss and MSE if requested
    ...
    def transform(self, graphs: List[nx.Graph]) -> List[np.ndarray]:
        """
        Transforms input NetworkX graphs into node-level embeddings using the trained GNN model.
        Each graph's embedding is a NumPy matrix with rows equal to the number of nodes and columns equal to the 'output' vector dimension.

        Parameters:
        - graphs: List of NetworkX graphs.

        Returns:
        - List of NumPy arrays, each corresponding to a graph's node embeddings.
        """
        if self.verbose >=1:
            print("Starting transformation of graphs into node-level embeddings...")
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(loader):
                ...
                # Split node outputs by graph in the batch
                ...
                for graph_idx in unique_graphs:
                    ...
                    graph_node_embed = out[node_indices]
                    embeddings.append(graph_node_embed)
                    
                if self.verbose >1:
                    print(f"Processed batch {batch_idx + 1}/{len(loader)}: Node Embeddings Shape {out.shape}")  # Debug
        if self.verbose >=1:
            print("Transformation complete. Node embeddings generated.")
        return embeddings
    ...
    def node_transform(self, graphs: List[nx.Graph]) -> List[np.ndarray]:
        ...
        """
        if self.verbose >=1:
            print("Starting node-level transformation using get_node_embeddings...")
        check_is_fitted(self, 'is_fitted_')
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(loader):
                ...
                node_embeds = self.model.get_node_embeddings(batch.x, batch.edge_index, batch.edge_attr, batch.batch)
                ...
                for graph_idx in unique_graphs:
                    node_indices = np.where(batch_graph_indices == graph_idx)[0]
                    graph_node_embed = node_embeds[node_indices]
                    embeddings.append(graph_node_embed)
                    
                if self.verbose >1:
                    print(f"Processed batch {batch_idx + 1}/{len(loader)}: Node Embeddings Shape {node_embeds.shape}")  # Debug
        if self.verbose >=1:
            print("Node-level transformation complete. Embeddings generated for all graphs.")
        return embeddings

    def fit_transform(self, graphs: List[nx.Graph], y: Optional[Union[np.ndarray, List]] = None) -> List[np.ndarray]:
        """
        Fits the model and transforms the graphs into node-level embeddings.
        ...
        Returns:
        - List of NumPy arrays containing node-level embeddings.
        """
        self.fit(graphs, y)
