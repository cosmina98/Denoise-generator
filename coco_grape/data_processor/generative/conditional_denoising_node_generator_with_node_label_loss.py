...
class IterativeDenoisingAutoencoderTransformerModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_edge_supervision and edge_idx.numel() > 0:
            ...
            # node_mask was part of the batch
            ...
            if valid.any():
                ...
                self.log("train_edge_loss" if self.training else "val_edge_loss", edge_loss, on_step=False, on_epoch=True)
                self.log("train_edge_acc"  if self.training else "val_edge_acc" , edge_acc , on_step=False, on_epoch=True)
        ...
        self.log("train_label_ce", losses["lab_ce"], on_step=False, on_epoch=True)   # (and val_...)
        return total_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.use_edge_supervision and edge_idx.numel() > 0:
            ...
            if valid.any():
                ...
                self.log("val_edge_loss" if self.training else "val_edge_loss", edge_loss, on_step=False, on_epoch=True)
                self.log("val_edge_acc"  if self.training else "val_edge_acc" , edge_acc , on_step=False, on_epoch=True)
        ...
        self.log("val_label_ce", losses["lab_ce"], on_step=False, on_epoch=True)   # (and val_...)
    ...
    def train_guidance_classifier(
        ...
        for _ in range(epochs):
            _, _, _, lat, _, _, _ = self.forward(X_tr, Y_tr, T_tr, return_latents=True, add_noise=False)
            ...
            with torch.no_grad():
                _, _, _, lat_val, _, _, _ = self.forward(X_val, Y_val, T_val, return_latents=True, add_noise=False)
    ...
    def generate(
        ...
        if use_heads_projection:
            ...
            if self.label_head is not None:
                with torch.no_grad():
                    _, _, _, logits_lab, _, _, _ = self.forward(x, global_condition, t0, return_latents=True, add_noise=False)
                lab_classes = torch.argmax(logits_lab, dim=-1)     # (B, N)
                self._last_lab_classes = lab_classes.detach().cpu()   # <-- store for wrapper
