...
class IterativeDenoisingAutoencoderTransformerModel(pl.LightningModule):
    ...
    def __init__(self,
        ...
        if self.use_distance_supervision:
            ...
            self.train_dist_loss = []; self.val_dist_loss = []
    ...
    def training_step(self, batch, batch_idx):
        ...
        if (getattr(self, "distance_head", None) is not None) and (dist_cls_idx.numel() > 0):
            ...
            if valid3.any():
                ...
                loss_dist = F.cross_entropy(logits_dist, dist_cls_lbl[valid3].long())
    ...
    def on_train_end(self):
        ...
        # NEW: include distance metrics if the head is active
        if getattr(self, "distance_head", None) is not None:
    ...
    def train_guidance_classifier(
        ...
        for _ in range(epochs):
            _, _, _, _, lat, _, _ = self.forward(X_tr, Y_tr, T_tr, return_latents=True, add_noise=False)
...
class ConditionalNodeGenerator:
    ...
    def setup(
        ...
        use_distance_supervision = (distance_pairs is not None) and (distance_targets is not None) and (len(distance_targets) > 0)
    ...
    def plot_metrics(self, window: int = 10, alpha: float = 0.3):
        ...
        # NEW: distance head plots
        if getattr(self.model, "distance_head", None) is not None:
