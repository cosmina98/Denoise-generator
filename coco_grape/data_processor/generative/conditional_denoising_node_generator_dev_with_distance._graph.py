...
class CrossTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, k, v, attn_bias: Optional[torch.Tensor] = None):
        x1 = self.norm1(x)

        mask = None
        if attn_bias is not None:
            # attn_bias: (B,N,N) -> (B*H,N,N)
            B, N, _ = attn_bias.shape
            H = self.self_attn.num_heads
            mask = attn_bias.repeat_interleave(H, dim=0)

        x = x + self.dropout1(self.self_attn(x1, x1, x1, attn_mask=mask)[0])
        x2 = self.norm2(x)
...
class EdgeMLP(nn.Module):
    def __init__(self, latent_dim: int, hidden_dim: Optional[int] = None,
                 dropout: float = 0.1, use_dist: bool = True, rbf_k: int = 0):
        super().__init__()
        if hidden_dim is None:
            hidden_dim = 2 * latent_dim
        in_dim = 4 * latent_dim + (1 if use_dist else 0) + (rbf_k if rbf_k > 0 else 0)
        ...
        self.rbf_k = rbf_k
        if rbf_k > 0:
            centers = torch.linspace(0, 4.0, rbf_k)
            self.register_buffer("rbf_mu", centers)
            self.register_buffer("rbf_gamma", torch.tensor(1.5))

    def forward(self, h_i, h_j):
        diff = torch.abs(h_i - h_j)
        prod = h_i * h_j
        feats = [h_i, h_j, diff, prod]
        if self.use_dist or self.rbf_k > 0:
            dist = torch.norm(h_i - h_j, dim=-1, keepdim=True)
            if self.rbf_k > 0:
                rbf = torch.exp(-self.rbf_gamma * (dist - self.rbf_mu.view(1, -1))**2)
                feats.append(rbf)
            if self.use_dist:
                feats.append(dist)
...
class IterativeDenoisingAutoencoderTransformerModel(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.use_smoothness = bool(use_smoothness)
    ...
    def forward(
        ...
        cont = x_norm.clone()
        cont[..., 0] = 0
        cont[..., self.important_feature_index] = 0

        # Pairwise distances in normalized feature space
        # returns (B,N,N)

        # RBF-like bias (negative and additive to logits)
        # stronger near t→0 so the model sharpens locally at the end
        attn_bias = None
        if self.use_distance_bias:
            with torch.no_grad():
                cont = x_norm.detach().clone()
                cont[..., 0] = 0
                cont[..., self.important_feature_index] = 0
                # pairwise squared distances: d2 = ||x||^2 + ||y||^2 − 2 x·y
                sq = (cont ** 2).sum(dim=-1, keepdim=True)           # (B,N,1)
                d2 = sq + sq.transpose(1, 2) - 2 * (cont @ cont.transpose(1, 2))  # (B,N,N)
                d2.clamp_(min=0)
                beta = self.distance_alpha * (1.0 - diffusion_time_step).view(-1, 1, 1)
                attn_bias = (- d2 / (2 * (self.distance_sigma ** 2) + 1e-8))
                attn_bias = (beta * attn_bias).to(latent_tokens.dtype)
        ...
        for layer in self.shared_transformer:
              latent_tokens = layer(latent_tokens, k=mem, v=mem, attn_bias=attn_bias)
        ...
        # x0 estimate; detach pred_eps in this path to prevent trivial shortcuts
        x0_hat = noisy_input - noise_scale * pred_eps.detach()


        # Re-encode x0_hat for cleaner head inputs (cheap: same encoder; no extra transformer pass)
        x0_latents = self.linear_encoder_input_to_latent(self.layernorm_in(x0_hat))

        # Heads run on the cleaner latents
        logits_deg   = self.degree_head(x0_latents)
        logits_exist = self.exist_head(x0_latents).squeeze(-1)
    ...
    def compute_weighted_loss(
        ...
        """
        Compute all weighted losses.
        - ε MSE only on continuous channels (mask out existence + degree)
        - Existence/degree heads are t-weighted (downweight very noisy steps)
        """
        pred_eps, logits_deg, logits_exist, eps, sigma_t = prediction

        # --- ε-loss on continuous channels only ---
        B, N, D = pred_eps.shape
        mask = torch.ones((1, 1, D), device=pred_eps.device, dtype=pred_eps.dtype)
        mask[..., 0] = 0  # existence channel
        mask[..., self.important_feature_index] = 0  # degree channel
        loss_eps = F.mse_loss(pred_eps * mask, eps * mask, reduction='mean')
                # ----- Consistency loss (optional) -----
        lam_cons = getattr(self, "lambda_consistency", 0.0)  # set >0 to enable
        if lam_cons > 0:
            # mask: continuous dims only (exclude existence & degree)
            cont_mask = torch.ones_like(target)
            cont_mask[..., 0] = 0
            cont_mask[..., self.important_feature_index] = 0

            # which rows should be near zero? those with target_exist == 0
            absent = (target[..., 0] < 0.5).float().unsqueeze(-1)

            # rebuild the same per-feature noise scale used in forward
            noise_scale = torch.ones_like(target) * sigma_t
            noise_scale[..., self.important_feature_index] /= self.noise_degree_factor

            # x_t = x0 + sigma*eps ;  x0_hat = x_t - sigma*pred_eps
            x_t     = target + noise_scale * eps
            x0_hat  = x_t - noise_scale * pred_eps  # <-- do NOT detach here
        ...
        # --- Targets ---
        # existence target from channel 0 (>= 0.5 means present)
        target_exist = (target[..., 0] >= 0.5).float()

        # degree target classes (unscale → round → clamp)
        ...
        true_deg_class = torch.clamp(torch.round(deg_unscaled), 0, self.max_degree).long()  # (B,N)

        # --- t-weights: smaller when sigma is big ---
        w = 1.0 / (sigma_t.squeeze(-1).squeeze(-1) ** 2 + 1e-4)  # (B,)

        # --- Existence BCE (per-node), then weight by w and mean ---
        exist_elem = F.binary_cross_entropy_with_logits(
            logits_exist, target_exist, pos_weight=self.exist_pos_weight, reduction='none'
        )  # (B,N)
        loss_exist = (exist_elem * w.view(-1, 1)).mean()

        # --- Degree CE (per-node), then weight by w and mean ---
        ...
        loss_deg_ce = (deg_ce_elem * w.view(-1, 1)).mean()
        ...
        total_loss = (
            ...
            # Optional distance-weighted smoothness on x0_hat (continuous dims only)
        if getattr(self, "use_smoothness", False) and self.lambda_smoothness > 0:
            cont_mask = torch.ones_like(target); cont_mask[..., 0] = 0; cont_mask[..., self.important_feature_index] = 0
            noise_scale = torch.ones_like(target) * sigma_t
            noise_scale[..., self.important_feature_index] /= self.noise_degree_factor
            x_t    = target + noise_scale * eps
            x0_hat = x_t - noise_scale * pred_eps

            x0c = x0_hat * cont_mask
            with torch.no_grad():
                tc = (target * cont_mask).detach()                     # (B,N,D)
                sq = (tc ** 2).sum(dim=-1, keepdim=True)               # (B,N,1)
                d2 = sq + sq.transpose(1, 2) - 2 * (tc @ tc.transpose(1, 2))  # (B,N,N)
                d2.clamp_(min=0)
                W = torch.exp(- d2 / (2 * (self.distance_sigma ** 2) + 1e-8))
            diffsq = ((x0c.unsqueeze(2) - x0c.unsqueeze(1)) ** 2).sum(-1)
            loss_smooth = (W * diffsq).mean()
            total_loss = total_loss + self.lambda_smoothness * loss_smooth
    ...
    def train_guidance_classifier(
        ...
        with torch.no_grad():
            pass  # ensure no lingering grads on X/Y
        ...
        for _ in range(epochs):
            _, _, _, lat, _, _ = self.forward(X_tr, Y_tr, T_tr, return_latents=True, add_noise=False)
            ...
            with torch.no_grad():
                _, _, _, lat_val, _, _ = self.forward(X_val, Y_val, T_val, return_latents=True, add_noise=False)
    ...
    def generate(
        ...
        with autocast_ctx:
            for i in range(total_steps - 1):
                ...
                # Consistent additive update: x_{next} = x_t - (sigma_t - sigma_{next}) * epŝ
...
class ConditionalNodeGenerator:
    ...
    def setup(
        ...
        self.model = IterativeDenoisingAutoencoderTransformerModel(
            ...
            use_smoothness=self.use_smoothness,
        ...
        try:
            ...
            if deg_classes is not None:
                ...
                for i in range(len(gen_orig)):
                    ...
                    gen_orig[i][..., deg_idx] = np.clip(deg_classes[i], 0, self.D_max)
