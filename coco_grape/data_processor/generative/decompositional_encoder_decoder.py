...
def _restore_numeric_label(label):
    """Convert decoded numeric-looking labels back to numeric values."""
...
class DecompositionalNodeEncoderDecoder(object):
    ...
    def decode(
        ...
        for graph_index, (encodings, node_labels, edge_labels, adj_mtx) in enumerate(zip(
            ...
            node_label_map = {
                i: _restore_numeric_label(label)
...
class DecompositionalEncoderDecoder(object):
    ...
    def __init__(
            ...
            use_direct_svd_encodings: bool = False,
            direct_node_svd_components: int = 128,
            direct_graph_svd_components: int = 128,
            direct_node_svd_keep_prefix: int = 3,
            direct_graph_svd_keep_prefix: int = 2,
            direct_svd_random_state: int = 42,
            ) -> None:
        """
        ...
        Parameters:
            ...
            use_direct_svd_encodings: Fit separate SVDs to raw node and graph
                encodings before the neural model. The compressed encodings are
                used directly and are not inverse-transformed before decoding.
        """
        ...
        self.use_direct_svd_encodings = bool(use_direct_svd_encodings)
        self.direct_node_svd_components = int(direct_node_svd_components)
        self.direct_graph_svd_components = int(direct_graph_svd_components)
        self.direct_node_svd_keep_prefix = int(direct_node_svd_keep_prefix)
        self.direct_graph_svd_keep_prefix = int(direct_graph_svd_keep_prefix)
        self.direct_svd_random_state = int(direct_svd_random_state)
        if self.direct_node_svd_components < 1 or self.direct_graph_svd_components < 1:
            raise ValueError("Direct SVD component counts must be positive")
        if self.direct_node_svd_keep_prefix < 0 or self.direct_graph_svd_keep_prefix < 0:
            raise ValueError("Direct SVD prefix lengths must be non-negative")
        self.direct_node_svd = None
        self.direct_graph_svd = None
        self._direct_svd_fitted = False
    ...
    def _fit_direct_svd(
        ...
        *,
        ...
        if values.ndim != 2:
            raise ValueError("Direct SVD expects a two-dimensional feature matrix")

        keep_prefix = min(max(0, int(keep_prefix)), values.shape[1])
        tail = values[:, keep_prefix:]
        if tail.shape[1] <= int(n_components) or tail.shape[0] < 2:
            return None
    ...
    def _apply_direct_svd(
        ...
        *,
        ...
        keep_prefix = min(max(0, int(keep_prefix)), values.shape[1])
        prefix = values[:, :keep_prefix]
        reduced_tail = reducer.transform(values[:, keep_prefix:])
        return np.hstack([prefix, reduced_tail])

    def _fit_direct_svd_encoders(
        self,
        node_encodings_list: List[np.ndarray],
        conditional_graph_encodings: Any,
    ) -> None:
        if not self.use_direct_svd_encodings:
            self.direct_node_svd = None
            self.direct_graph_svd = None
            self._direct_svd_fitted = False
            return
        ...
        node_rows = np.vstack([
            np.asarray(enc, dtype=float)
            for enc in node_encodings_list
        ])
        condition_rows = np.asarray(conditional_graph_encodings, dtype=float)
        if condition_rows.ndim == 1:
            condition_rows = condition_rows.reshape(1, -1)

        self.direct_node_svd = self._fit_direct_svd(
            node_rows,
            keep_prefix=self.direct_node_svd_keep_prefix,
            n_components=self.direct_node_svd_components,
            random_state=self.direct_svd_random_state,
        )
        self.direct_graph_svd = self._fit_direct_svd(
            condition_rows,
            keep_prefix=self.direct_graph_svd_keep_prefix,
            n_components=self.direct_graph_svd_components,
            random_state=self.direct_svd_random_state,
        )
        self._direct_svd_fitted = True

        node_width = self._apply_direct_svd(
            node_rows[:1],
            reducer=self.direct_node_svd,
            keep_prefix=self.direct_node_svd_keep_prefix,
        ).shape[1]
        condition_width = self._apply_direct_svd(
            condition_rows[:1],
            reducer=self.direct_graph_svd,
            keep_prefix=self.direct_graph_svd_keep_prefix,
        ).shape[1]
        if self.verbose:
            print(
                "Direct SVD encodings fitted: "
                f"node {node_rows.shape[1]} -> {node_width}, "
                f"condition {condition_rows.shape[1]} -> {condition_width}. "
                "Compressed encodings will not be inverse-transformed."
            )

    def _transform_direct_node_encodings(
        self,
        node_encodings_list: List[np.ndarray],
    ) -> List[np.ndarray]:
        if not self.use_direct_svd_encodings or not self._direct_svd_fitted:
            return [np.asarray(enc, dtype=float) for enc in node_encodings_list]
        return [
            self._apply_direct_svd(
                enc,
                reducer=self.direct_node_svd,
                keep_prefix=self.direct_node_svd_keep_prefix,
            )
    ...
    def _transform_direct_graph_encodings(
        self,
        conditional_graph_encodings: Any,
    ) -> np.ndarray:
        condition_rows = np.asarray(conditional_graph_encodings, dtype=float)
        ...
        return self._apply_direct_svd(
            condition_rows,
            reducer=self.direct_graph_svd,
            keep_prefix=self.direct_graph_svd_keep_prefix,
        )

    def _disable_nested_reducers_for_direct_svd(self) -> None:
        if not self.use_direct_svd_encodings:
            return
        cond_model = self.conditioning_to_node_embeddings_generator
        generator = getattr(cond_model, "conditional_node_generator", None)
        if generator is None:
            return

        disabled = []
        for attribute in ("use_dim_reduction", "use_condition_dim_reduction"):
            if bool(getattr(generator, attribute, False)):
                setattr(generator, attribute, False)
                disabled.append(attribute)
        if disabled and self.verbose:
            print(
                "Direct SVD encodings are enabled; disabled nested generator "
                f"reducers: {', '.join(disabled)}."
            )
    ...
    def fit(
        ...
        self._sync_distance_supervision_flag()
        self._disable_nested_reducers_for_direct_svd()

        # Fit vectorizers
        self.graph_vectorizer.fit(graphs)
        self.node_graph_vectorizer.fit(graphs)

        # Generate raw encodings once, fit the optional direct SVDs on the
        # training set only, then train every downstream component in the
        # resulting compressed space.
        self.direct_node_svd = None
        self.direct_graph_svd = None
        self._direct_svd_fitted = False
        node_encodings_list, conditional_graph_encodings = self.encode(graphs)
        self._fit_direct_svd_encoders(
            node_encodings_list,
            conditional_graph_encodings,
        )
        node_encodings_list = self._transform_direct_node_encodings(
            node_encodings_list
        )
        conditional_graph_encodings = self._transform_direct_graph_encodings(
            conditional_graph_encodings
        )
        if self.node_embeddings_to_graph_generator is not None:
            self.node_embeddings_to_graph_generator._last_conditioning_vectors = list(
                conditional_graph_encodings
            )
    ...
    def graph_encode(self, graphs: List[nx.Graph]) -> List[np.ndarray]:
        ...
        raw_encodings = self.graph_vectorizer.transform(graphs)
        return self._transform_direct_graph_encodings(raw_encodings)
    ...
    def fit_classifier(self, graphs, targets, epochs=20, lr=1e-3):
        ...
        node_encs, cond_vecs = self.encode(graphs)
