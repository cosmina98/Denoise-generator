...
class DecompositionalEncoderDecoder(object):
    ...
    def __init__(
            ...
            use_anchor_hops: bool = False,
            anchor_hops_K: int = 6,
            anchor_hops_Hmax: int = 3,
            anchor_hops_strategy: str = "degree", # "degree" | "pagerank" | "random"
            anchor_hops_encoding: str = "onehot",   # "onehot" | "rbf"
            anchor_hops_rbf_gamma: float = 1.0
            ) -> None:
        """
        ...
        self.use_anchor_hops = use_anchor_hops
        self.anchor_hops_K = anchor_hops_K
        self.anchor_hops_Hmax = anchor_hops_Hmax
        self.anchor_hops_strategy = anchor_hops_strategy
        self.anchor_hops_encoding = anchor_hops_encoding
        self.anchor_hops_rbf_gamma = anchor_hops_rbf_gamma
    ...
    def _select_anchors(self, G: nx.Graph, K: int, strategy: str) -> List[Any]:
       nodes = list(G.nodes())
       if len(nodes) <= K:
           return nodes
       if strategy == "degree":
           # highest-degree nodes
           return [u for u, _ in sorted(G.degree, key=lambda t: t[1], reverse=True)[:K]]
       if strategy == "pagerank":
           pr = nx.pagerank(G) if G.number_of_edges() else {u: 0.0 for u in nodes}
           return [u for u, _ in sorted(pr.items(), key=lambda t: t[1], reverse=True)[:K]]
       # fallback: random distinct anchors
       rng = np.random.default_rng(42)
       return list(rng.choice(nodes, size=K, replace=False))
    def _onehot_bins(self, d: int, Hmax: int) -> np.ndarray:
       idx = min(int(d), Hmax)  # bin Hmax == ">= Hmax"
       vec = np.zeros(Hmax + 1, dtype=float)
       vec[idx] = 1.0
       return vec

    def _rbf_bins(self, d: float, Hmax: int, gamma: float) -> np.ndarray:
       centers = np.arange(Hmax + 1, dtype=float)
       return np.exp(-gamma * (d - centers) ** 2)

    def _anchor_hops_features(
       self, G: nx.Graph, node_order: List[Any], K: int, Hmax: int,
       encoding: str, rbf_gamma: float
    ) -> np.ndarray:
       """Return (N, K*(Hmax+1)) features per node: hop distances to each anchor."""
       anchors = self._select_anchors(G, K, self.anchor_hops_strategy)
       N = len(node_order)
       idx_of = {u: i for i, u in enumerate(node_order)}
       feats_per_anchor = []
       for a in anchors:
           # shortest-path hops from anchor 'a' (cap at Hmax+ for unreachable)
           dists = {u: Hmax + 1 for u in node_order}
           for u, d in nx.single_source_shortest_path_length(G, a, cutoff=Hmax).items():
               dists[u] = min(d, Hmax + 1)  # unreachable remains Hmax+1
           # treat "unreachable" as Hmax bin
           vecs = []
           for u in node_order:
               d = dists[u]
               d = Hmax if d > Hmax else d
               if encoding == "rbf":
                   vecs.append(self._rbf_bins(d, Hmax, rbf_gamma))
               else:
                   vecs.append(self._onehot_bins(d, Hmax))
           feats_per_anchor.append(np.vstack(vecs))  # (N, Hmax+1)
       return np.hstack(feats_per_anchor) if feats_per_anchor else np.zeros((N, K * (Hmax + 1)))

    def _augment_with_anchor_hops(
       self, graphs: List[nx.Graph], node_encodings_list: List[np.ndarray]
    ) -> List[np.ndarray]:
       """Append anchor-hop features after existing columns (keeps exist @0, degree @1)."""
       augmented = []
       for G, X in zip(graphs, node_encodings_list):
           order = list(G.nodes())
           hop_feats = self._anchor_hops_features(
               G, order, self.anchor_hops_K, self.anchor_hops_Hmax,
               self.anchor_hops_encoding, self.anchor_hops_rbf_gamma
           )
           # If your encoder's node order matches G.nodes() (it does elsewhere),
           # rows align. Otherwise reorder here.
           augmented.append(np.hstack([X, hop_feats]))
       return augmented
    ...
    def encode(self, graphs: List[nx.Graph]) -> Tuple[List[np.ndarray], Any]:
        ...
        if self.use_anchor_hops:
            node_encs = self._augment_with_anchor_hops(graphs, node_encs)
