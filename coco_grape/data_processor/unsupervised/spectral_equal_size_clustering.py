...
From: https://github.com/anamabo/Equal-Size-Spectral-Clustering/
"""
import numpy as np
import pandas as pd
...
class SpectralEqualSizeClustering:
    ...
    def __init__(self, nclusters: int = None, nneighbors: int = None, equity_fraction=0.9, seed=None):
    ...
    def cluster_equalization(self, dmatrix):
        ...
        if (len(large_clusters) == 0) & (len(small_clusters) == 0):
            ...
            self.total_cluster_dispersion = self.final_cluster_dispersion.sum(axis=0)
...
class EqualSizeSpectralClustering(object):
    def __init__(self, n_clusters, n_neighbors=None, equity_fraction=0.9, estimator=ExtraTreesClassifier(n_estimators=300)):
    ...
    def fit(self, data_mtx, targets=None):
        distance_mtx = pairwise_distances(data_mtx)
        if self.n_neighbors is None: self.n_neighbors = max(int(len(data_mtx)*0.1), 5)
        cluster_labels = SpectralEqualSizeClustering(nclusters=self.n_clusters, nneighbors=self.n_neighbors, equity_fraction=self.equity_fraction).fit(distance_mtx)
        self.estimator.fit(data_mtx, cluster_labels)
        return self

    def predict(self, data_mtx):
        preds = self.estimator.predict(data_mtx)
        return preds

    def fit_predict(self, data_mtx, targets=None):
        return self.fit(data_mtx).predict(data_mtx)
