...
continuous generative models provide stable denoising objectives, but graphs are
discrete, sparse, permutation-sensitive, and subject to structural constraints.
This chapter addresses that difficulty by introducing the
\emph{Graph-Conditioned Denoising Generator} (GCDG), the
generator used in this thesis.

The central claim is that graph-level conditional generation becomes more
controllable when continuous denoising and discrete graph decoding are
not forced into a single neural prediction problem. GCDG therefore learns to
generate node-level representations conditioned on a graph-level description,
and then constructs a discrete graph through learned structural predictions
and an explicit constraint-aware decoding step.

This separation is useful for the setting of this thesis because the
conditioning information is graph-level, whereas the output is a variable-size
graph with node labels, edge labels, degree constraints, and, in some datasets,
additional domain constraints. The method therefore does not treat the
conditioning vector as a class label. It treats it as a continuous description
of the requested graph. Such a condition may be instantiated using explicit
graph-kernel feature maps, including neighbourhood or propagation-based kernels
\citep{costa2010fast,neumann2016propagation,borgwardt2020graphkernels}, or by
learned graph-level embeddings from graph neural networks and graph
Transformers \citep{hamilton2017graphsage,zhang2018endtoend,dwivedi2021graph}.
Generation is then the task of producing a graph whose decoded structure is
compatible with that graph-level description.

This is a conditional generation setting rather than an unconditional sampling
setting. When \(C\) is computed from a reference graph, the model is not given
the reference graph itself and is not asked to reproduce that exact graph by
identity. It receives only the graph-level vector \(C\). The objective is
therefore to generate a graph that preserves the information encoded by the
condition. Since \(C\) can be computed for graphs that were not present during
training, the evaluation can use unseen condition vectors. This differs from a
graph autoencoder or variational graph autoencoder, which maps a specific input
graph to a sample-specific latent code and then decodes from that code
\citep{kipf2016vgae,simonovsky2018graphvae}. GCDG instead maps an external
graph-level descriptor to a new graph. It may be evaluated against a reference
graph that supplied the descriptor, but the target is compatibility with the
encoded structural and label information, not exact reconstruction of the
reference adjacency matrix.

The approach builds on the denoising principle introduced in
Chapter~\ref{ch:theory} and used in diffusion and score-based generative models
\citep{ho2020ddpm,song2020score}. However, it is not a \emph{direct graph
diffusion model}: the adjacency matrix and categorical edge states are not the
primary variables evolved by the reverse process. Continuous graph diffusion
methods perturb node features and adjacency tensors in a relaxed space
\citep{niu2020permutation,jo2022score}, while discrete graph diffusion methods
such as DiGress define Markov transitions over categorical node and edge states
\citep{vignac2022digress}. GCDG takes a different route: it applies denoising
to padded node-level representations, then enforces discrete topology during a
separate constrained graph-decoding stage. The rest of this chapter motivates
this separation and then defines the representation, denoising model,
training objective, sampling procedure, and graph decoder.

\section{Design Rationale: A Staged Conditional Graph Generator}
\label{sec:generator:motivation}

Graph generation is difficult because the output space is simultaneously
discrete, permutation-sensitive, sparse, and constrained. A generator must
decide how many nodes exist, which labels they carry, which edges connect them,
and which edge labels are valid. These decisions are not independent: the degree
of a node restricts how many incident edges can be selected, node labels may
restrict which edge labels are meaningful, and global graph properties depend
on many local choices. This coupling is one reason why graph generators often
avoid a single unconstrained output layer, instead using either explicit
autoregressive factorisations \citep{you2018graphrnn,liao2019gran} or
specialised graph decoders in latent-variable models
\citep{simonovsky2018graphvae}.

The generator proposed in this chapter is staged. First, it learns to produce a
node-level representation conditioned on a graph-level vector. Second, it
predicts structural quantities from that representation. Third, it decodes those
predictions into a discrete graph. In this context, \emph{denoising} means
learning to recover a clean node-level representation from a corrupted version
of that representation. \emph{Graph decoding} means the later step that turns
node predictions and pairwise scores into an explicit graph with nodes, edges,
and labels.

This staging is motivated by diffusion-based models, which offer a stable
alternative to adversarial or purely autoregressive training, but the graph
setting introduces an additional modelling choice: what object should be noised
and denoised? Continuous score-based graph generators perturb node features and
adjacency tensors in a relaxed space
\citep{niu2020permutation,jo2022score}. This is expressive, but the noisy
adjacency is no longer a sparse discrete graph, so structural quantities such
as exact degrees, connectivity, or cycles are not directly preserved along the
trajectory. Discrete graph diffusion models, such as DiGress, instead define
categorical transitions over node and edge states and therefore keep the
denoising state graph-valued \citep{vignac2022digress}. The generator proposed
in this chapter takes a third route. It uses denoising for conditional
node-level representation learning, then recovers the discrete graph through
learned structural predictions and a constrained decoding step.

The design problem addressed in this chapter is therefore not simply how to add
noise to a graph. It is how to use a continuous graph-level condition to control
a discrete, variable-size object without losing the structural constraints that
make the output a graph. The proposed design separates these roles. The
denoising model learns a conditional distribution over node-level
representations; pairwise predictors estimate which node pairs should be
connected; and the decoder converts these predictions into a feasible graph.
This separation makes the intermediate objects inspectable, while allowing
strict constraints such as requested cycle size, degree consistency,
connectivity, molecular validity, or novelty to be evaluated at the level of
...
The feature vectors are stacked and padded to form the clean node feature
matrix
\[
    X_0
    =
...
\section{Forward Corruption Process}
...
The denoiser is a transformer over node rows. This choice follows the use of
...
\([z_i;z_j]\) denotes vector concatenation. When edge labels are available, a
second pairwise classifier predicts a distribution
\(p^{\tau_E}_{ij}\in\Delta^{|\mathcal{T}_E|-1}\) over the edge-label classes
for each supervised pair. When distance supervision is enabled, another
...
information part of the denoising trajectory itself. The purpose is to let
...
currently likely to be adjacent to it. In matrix form, this optional refinement
can be written as
...
The training-time corruption can be read as an edge analogue of the node-feature
corruption. Starting from the clean adjacency \(A_0\), the model removes some
true edges and inserts some false edges to obtain \(A_t^{\mathrm{state}}\). At
small noise levels, this state remains close to the true graph; at large noise
levels, it becomes a rougher provisional structure. The denoiser must therefore
learn to use edge information when it is helpful while remaining robust to
structural mistakes in the provisional graph.
...
state variable that can influence subsequent denoising steps.
...
At a denoising step \(t\), let \(M_t^{\mathrm{mol}}\in
...
Here \(B_{\mathrm{cont}}\) masks out feature dimensions handled by separate categorical
heads, such as node existence, degree, and label, and
\(\eta_{\mathrm{num}}>0\) is a small numerical stabiliser. The clean-matrix estimate is

\[
    \hat{X}_0 = X_t - \Gamma(t)\odot \hat{\varepsilon}.
\]

An optional direct clean-matrix loss can be used to anchor the denoising process:
...
Node existence is trained using binary cross-entropy:
...
\(\mathcal{L}_{\mathrm{dist}}\). The full loss can be written as
...
The consistency term discourages non-existent padded rows from carrying
non-zero continuous information. The optional condition-to-\(X_0\) head predicts
a clean node feature matrix directly from the condition and can be used either as an
auxiliary training target or as a sampling prior, but it is not required for the
main generator.

\section{Sampling and Backward Process}
\label{sec:generator:sampling}

The backward process defines conditional generation from a graph-level
...
prior prediction; otherwise, the reverse process begins from the neutral noisy
state.

The reverse trajectory follows a decreasing sequence of noise levels
...
The denoising vector field is applied only to continuous feature dimensions.
Discrete node attributes are recovered through their supervised prediction
modules. Thus, after the final denoising step, node existence is obtained by
...
state is updated throughout the reverse trajectory; otherwise, edge
probabilities are computed from the final node latents before graph decoding.

\begin{algorithm}[H]
\caption{Reverse conditional generation of the node feature matrix}
...
\For{\(t=T,\ldots,1\)}
    \State Predict denoising direction and discrete node attributes using \(F_\theta(X_t,C,t)\)
    \State Update continuous feature dimensions of \(X_t\) using Euler--Heun denoising
...
set of graphs satisfying the required structural constraints. This idea follows
the broader structured-prediction and combinatorial-optimisation view that a
learning model can provide scores for a discrete decision problem, while an
optimisation procedure enforces feasibility
\citep{donti2017task,bengio2021combinatorial}. It is also related in spirit to
constraint-aware molecular and graph generation, where validity is not left
entirely to an unconstrained output layer
\citep{liu2018constrained,you2018gcpn,samanta2020nevae}.
...
Given an edge-probability matrix \(P^{\mathrm{edge}}\) and target degrees
\(\hat{g}_1,\ldots,\hat{g}_n\), adjacency decoding is posed as a binary
...
Here \(u_i\) and \(v_i\) are non-negative slack variables. They allow the solver
to recover a graph even when the predicted degree sequence is not exactly
graphical, while penalising deviations strongly. When connectivity is required,
additional flow constraints enforce a single connected component. The solver is
warm-started with a maximum spanning tree over the edge probabilities.
...
\section{Relation to Direct Graph Diffusion}
\label{sec:generator:relation}

The proposed generator differs from direct graph diffusion models in where the
stochastic process is placed. Direct discrete graph diffusion models typically
diffuse node and edge classes directly and learn a reverse process over the
complete graph state. In contrast, this generator denoises a continuous
node-feature representation and uses pairwise heads plus a constrained decoder to
recover the final graph. The adjacency matrix is therefore not the primary
diffused object. It is predicted from node latents and then selected under
constraints.

This places the method between several existing families of graph generators.
Variational, adversarial, and flow-based graph generators model discrete graphs
through latent-variable objectives, implicit adversarial training, or invertible
transformations \citep{kusner2017gvae,simonovsky2018graphvae,de2018molgan,
madhawa2019graphnvp,zang2020moflow,lippe2022graphcnf}. Continuous graph
diffusion methods instead add Gaussian noise to relaxed node and adjacency
representations \citep{niu2020permutation,jo2022score}. DiGress avoids this
relaxation by using a discrete diffusion process over categorical node and edge
states \citep{vignac2022digress}. Molecular diffusion models in three
dimensions address a related but distinct task: they generate atom coordinates
or conformers rather than graph topology, and therefore require geometric
training data \citep{hoogeboom2022equivariant}. GCDG is closest in motivation
to conditional graph generation, but it does not diffuse the adjacency matrix
or categorical edge state directly. It denoises a conditioned node feature
matrix and delegates graph feasibility to the decoder.

The distinction from variational graph autoencoders is especially important.
A graph \gls{vgae} learns an encoder--decoder pathway
\[
    G \mapsto z \mapsto \hat G,
\]
where \(z\) is a latent variable inferred from the input graph. Conditional
generation in GCDG follows a different pathway,
\[
    C(G_{\mathrm{ref}}) \mapsto \hat X_0 \mapsto \hat G,
\]
where \(C(G_{\mathrm{ref}})\) is an externally computed graph-level condition.
The original graph \(G_{\mathrm{ref}}\) is not passed through a graph encoder
inside the generator during sampling. Consequently, when a held-out condition is
used, the question is not whether the model can reconstruct the exact held-out
graph, but whether it can generate a graph whose decoded structure is
compatible with the information retained by \(C\).

This distinction is important for interpreting comparisons with conditional
DiGress-style models. A conditional DiGress adaptation places the graph-level
condition into the model's global feature channel and samples node and edge
classes through a discrete reverse process. The generator in
this chapter instead places the graph-level condition into cross-attention
memory and learns a conditional denoising process over padded node rows. Both
models can be conditioned on the same graph-level vector \(C\), but they use
different generative mechanisms.

The benefit of this staged design is that it exposes intermediate
objects that can be inspected: node counts, degree histograms, label histograms,
edge probability matrices, and constraint-solver outputs. The cost is that the
final graph quality depends not only on the neural denoiser, but also on the
choice of representation and decoder.
Implementation-level settings and ablation parameters are not part of the
method claim itself; they are tabulated separately in
