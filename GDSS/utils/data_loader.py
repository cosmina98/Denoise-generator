...
from utils.graph_utils import graph_labels_to_tensor, init_features, graphs_to_tensor


def graphs_to_dataloader(config, graph_list):

    adjs_tensor = graphs_to_tensor(graph_list, config.data.max_node_num)
    if config.data.init == 'label':
        x_tensor = graph_labels_to_tensor(
            graph_list,
            config.data.max_node_num,
            config.data.max_feat_num,
        )
    else:
