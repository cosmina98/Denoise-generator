...
def graphs_to_tensor(graph_list, max_node_num):
    ...
    for g in graph_list:
        ...
        adj = nx.to_numpy_array(g, nodelist=node_list, dtype=np.float32)
...
def graph_labels_to_tensor(graph_list, max_node_num, num_classes, label_key='label'):
    label_tensors = []

    for graph in graph_list:
        assert isinstance(graph, nx.Graph)
        node_list = list(graph.nodes())
        labels = torch.zeros((max_node_num, num_classes), dtype=torch.float32)

        for row, node in enumerate(node_list):
            label = int(graph.nodes[node].get(label_key, 0))
            if label < 0 or label >= num_classes:
                raise ValueError(
                    f"Node label {label} is outside [0, {num_classes - 1}]"
                )
...
def graphs_to_adj(graph, max_node_num):
    ...
    adj = nx.to_numpy_array(graph, nodelist=node_list, dtype=np.float32)
    padded_adj = pad_adjs(adj, node_number=max_node_num)
