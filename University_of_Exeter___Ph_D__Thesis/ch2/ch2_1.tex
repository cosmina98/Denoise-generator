...
A labelled graph is a graph $G=(V,E)$ equipped with a labeling function
...
A key methodological distinction concerns \emph{transductive} versus \emph{inductive} settings. In transductive learning, all nodes are observed during training but only a subset is labelled. In inductive learning, the model must generalize to previously unseen nodes or entirely new graphs. Because GNNs learn local aggregation rules rather than node-specific parameters, many architectures naturally support inductive generalisation.
...
evaluation as the optimisation of a single numerical score or relying on
...
that are interpretable, resistant to memorisation artifacts, and applicable
