...
and attribute information, together with the mathematical notation used
...
Graphs are a fundamental data structure for modelling relationships among entities
...
a powerful modelling formalism, but also introduces significant challenges for
representation, learning, generation, and evaluation.
\begin{definition}[Graph]
A (finite) graph is a tuple $G=(V,E)$, where
$V=\{v_1,\dots,v_n\}$ is a finite set of vertices and \(E\) is a finite
set of edges. If $G$ is \emph{undirected}, edges are unordered pairs
$\{u,v\}$ with \(u,v\in V\); if $G$ is \emph{directed}, edges are ordered
...
\(\pi\) preserves adjacency: vertices \(u\) and \(v\) are adjacent in \(G\)
if and only if \(\pi(u)\) and \(\pi(v)\) are adjacent in \(G'\). Graph
isomorphism formalizes structural equivalence independently of
vertex ordering.
\end{definition}
\begin{figure}[h]
\centering
...
\end{tikzpicture}
\caption{(a) Unlabelled undirected graph. (b) Fully labelled graph (node and edge labels). (c) Attributed graph with real-valued node features.}
...
\emph{node-labelled} graph, while a graph whose edges carry labels is termed
\emph{edge-labelled}. When both vertices and edges are labelled, the graph is
\emph{discretely labelled}. Figure~\ref{fig:graph-types} illustrates several common
graph variants considered in this thesis, including an undirected graph, a fully
labelled graph, and a graph with real-valued node attributes.

\begin{definition}[Labelled graph]
A labelled graph is a graph \(G=(V,E)\) equipped with a node-label function
\(\tau_V:V\to\mathcal{T}_V\) and, optionally, an edge-label function
\(\tau_E:E\to\mathcal{T}_E\), where \(\mathcal{T}_V\) and
\(\mathcal{T}_E\) are finite sets of discrete labels.
\end{definition}
...
\(\{u,v\}\) or \((u,v)\). Attributes may be continuous or categorical.
\end{definition} 
...
\[
N(v_i)=\{\,v_j\in V \mid v_i \text{ and } v_j \text{ are adjacent}\,\},
\]
...
$A_{ij}=1$ if \(v_i\) and \(v_j\) are adjacent and $A_{ij}=0$ otherwise;
for weighted graphs, $A_{ij}$ may store an edge weight.
\end{definition}

\paragraph{Neighbourhood and degree.}
...
\begin{aligned}
G(S) &= (S,E(S)), \\
E(S) &= \{e\in E \mid \text{all endpoints of } e \text{ belong to } S\}.
\end{aligned}
\]
\paragraph{Shortest paths and neighbourhoods.}
...
$r$ is an integer.
These neighbourhoods capture increasingly large local regions of the graph and
...
arbitrary: a relabelling of the vertices should not change the underlying
...
graph under the relabelled ordering has adjacency matrix
\[
...
\begin{figure*}[h]
    ...
    representations are obtained through data-driven optimisation, including
...
statistics, or patterns---such as walks, paths, neighbourhoods, subgraphs, or
...
continuous attributes without discretisation.
...
The relabelling procedure requires
...
discretisation or additional adaptations.
...
handled via discretisation or hashing,
...
compares graphs by modelling their structure
...
functions. Sampling $h\sim\mathcal{H}$ induces a discretely labelled graph $h(G)$ by setting
...
randomized discretisation. Their main limitation is that performance depends on the chosen
...
than through stochastic discretisation.
\subsection{Learned representations}

In contrast to explicit representations, learned representations construct
graph embeddings through data-driven optimisation. Rather than fixing a feature
map \(\phi\) a priori, these methods learn a parameterised mapping
\(\phi_\theta : \mathcal{G} \to \mathbb{R}^d\) whose parameters are optimised with
...
and the optimisation objective, they are typically more flexible than explicit
...
structure, enabling scalable optimisation on large graphs.
...
Deep neural network–based models have emerged as a dominant paradigm for graph representation learning, driven by advances in deep learning and their ability to jointly model graph structure and node or edge attributes. These models extend classical embedding techniques by learning representations through parameterised neural architectures, enabling inductive generalisation to unseen nodes and graphs. The most prominent class within this family is \emph{Graph Neural Networks} (GNNs), which learn node- or graph-level embeddings by iteratively aggregating and transforming information from local neighbourhoods.
...
GNNs can be trained under node, edge, or graph-level supervision. In node classification, a predictor $p_\theta(y_v \mid \mathbf{h}_v^{(L)})$ (e.g., softmax layer) is optimised on labelled nodes. In link prediction, a similarity function or multilayer perceptron operates on pairs $(\mathbf{h}_u^{(L)},\mathbf{h}_v^{(L)})$. In graph classification, predictions are made from $\mathbf{z}_G$.

A key methodological distinction concerns \emph{transductive} versus \emph{inductive} settings. In transductive learning, all nodes are observed during training but only a subset is labelled. In inductive learning, the model must generalise to previously unseen nodes or entirely new graphs. Because GNNs learn local aggregation rules rather than node-specific parameters, many architectures naturally support inductive generalisation.
...
Kernels emphasise pairwise comparison by defining an explicit similarity
...
optimisation procedures, kernel methods often provide stronger
...
individual graphs or nodes through parameterised models trained on
task-specific objectives. These approaches are well suited for large-scale
prediction, representation transfer, and generative modelling pipelines,
where joint optimisation and inductive generalisation are desirable.Importantly, the choice between kernels and learned embeddings is not
...
\begin{figure}[H]
    ...
    \caption{
    Overview of major deep generative modelling paradigms for graphs.
...
graph generation requires modelling the full joint distribution over
...
likelihood-based deep generative modelling paradigms.
...
bond interactions in molecular graphs. While normalising flows provide principled density modelling
...
learning a parameterised reverse process that inverts a fixed
Gaussian noising schedule. \textbf{Stochastic Differential Equations } (Score SDEs)
\citep{song2020score} generalise discrete diffusion processes
to stochastic differential equations (SDEs), modelling the forward
...
optimised by maximizing a variational lower bound on $\log p_\theta(x_0)$, which
...
Their appeal lies in stable optimisation objectives, strong empirical
...
Existing approaches can be organised according to how the diffusion
...
An alternative line of work adopts score-based generative modelling
...
modelling directly to molecular conformations in continuous coordinate
space.

\textbf{Continuous-time SDE formulations.}
Score-based diffusion can further be generalised to continuous time
...
(iii) the modelling of variable graph size; and
...
post-hoc discretisation; direct diffusion over adjacency often
...
In domains such as image generation, visual inspection is a useful diagnostic but an unreliable proxy for generalisation or downstream utility\citep{theis2016note} ; evaluation should therefore be aligned with the intended application and
...
generated data generalise to real test sets. This framing directly links evaluation to
...
supports downstream predictive modelling tasks. Motivated by these limitations, recent work has increasingly shifted attention from
...
Rather than collapsing performance to a single scalar score, these approaches emphasise
...
consistency and organises them into structured ``evaluation exercises.''
...
These limitations motivate a shift in perspective. Instead of treating
evaluation as the optimisation of a single score, the thesis asks whether model
rankings remain stable across datasets, probes, and evaluation criteria. In
Chapter~\ref{ch:rankgen}, this is done by defining predictive tasks on real and
generated data, interpreting their empirical errors through PAC-style
reliability bounds, and ranking only the comparisons that pass those checks.
This formulation does not guarantee that one generator is universally best; it
specifies when the reported evidence is reliable under the chosen probes. It
also separates failure modes such as distributional shift, loss of diversity,
memorisation, and poor predictive utility, rather than collapsing them into one
scalar value.
...
probabilistic generalisation guarantees in the
...
to derive generalisation guarantees for classifier-based evaluation metrics,
