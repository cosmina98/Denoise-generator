\chapter{RankGen: Statistically Robust Evaluation of Generative Models}
\label{ch:rankgen}
...
assign high scores to models that memorize training data, fail to generalize,
...
These limitations motivate the need for evaluation frameworks that are
diagnostic, statistically grounded, and capable of distinguishing between
different failure modes of generative models. In the next section we therefore
reformulate generative model evaluation as a learning problem, which provides
the conceptual foundation for the RankGen framework.

\section{Overview of This Chapter's Contributions}
To address these challenges, this chapter introduces \textbf{RankGen}, a classifier-based evaluation framework that reframes generative model assessment as a collection of predictive probes targeting complementary failure modes. Rather than relying on a single scalar metric, RankGen evaluates generators along multiple axes. The framework defines four metrics: \emph{Quality} (signal fidelity), \emph{Utility} (added predictive value beyond real data), \emph{Indistinguishability} (two-sample alignment), and \emph{Similarity} (local structural mixing between real and generated samples). Each metric is paired with a PAC-style generalisation bound, transforming empirical performance estimates into statements that hold with high probability. RankGen evaluates candidate generators through a two-stage protocol: first, models that violate statistical reliability bounds are filtered out; second, the remaining models are ranked using robust quantile-based summaries that account for variability across repeated evaluations. A composite score, termed \emph{Exchangeability}, aggregates the resulting signals to capture both predictive usefulness and distributional alignment. 

The main contributions of this chapter are as follows. We introduce a statistically grounded evaluation framework that reformulates generative model assessment as a collection of predictive probes. We propose four complementary classifier-based metrics equipped with PAC-style guarantees that diagnose distinct failure modes of generative models. We develop a four stages evaluation protocol that combines statistical reliability checks with robust quantile-based ranking. We provide empirical studies demonstrating that RankGen exposes important pathologies, including memorisation, that conventional metrics often miss. This chapter constitutes the third methodological contribution of the thesis; while earlier chapters focused on representation learning and generative modelling for structured data, the present chapter addresses the equally important problem of evaluating generative models in a statistically principled manner.

\paragraph{Chapter structure.}
The remainder of this chapter is organised as follows.
Section~\ref{sec:rankgen:limitations} reviews the limitations of existing
evaluation approaches and motivates the need for a statistically grounded,
...
For example, if a classifier trained on generated data generalizes poorly to
...
$\rho_6^{(i)} = \Gamma_{\theta}(D_{\text{train1}}^{(i)}, D_{\text{generated}}^{(i)})$ & Entropy-based similarity of local feature neighborhoods. \\
\bottomrule
\end{tabular}
\end{small}
\end{table}
...
value beyond the original real training set. When a generator merely memorizes
...
\textbf{Indistinguishability}, designed to capture \textit{real-vs-fake separability}, often fails under copy scenarios where discriminators do not generalize well.
...
To assess whether \textsc{RankGen} generalizes beyond graph and image generators, we evaluate it on open-domain text generation using the \textsc{AG-News} corpus~\citep{zhang2015character}, which contains news headlines labelled with four topics (\emph{World}, \emph{Sports}, \emph{Business}, \emph{Sci/Tech}). We treat human-written headlines as real data and consider three GPT-2 variants~\citep{radford2019language} as conditional generators: \texttt{distilgpt2}, \texttt{gpt2}, and \texttt{gpt2-medium}. For each class, models are prompted with a topic-specific prefix and used to sample synthetic headlines.
...
These results show that \textsc{RankGen} generalizes naturally to high-dimensional text generation without any text-specific modifications to the method.
\section{Conclusion}
