...
$G=(V,E,\tau_V,\tau_E)$ & Labelled graph with node-label function \(\tau_V\) and edge-label function \(\tau_E\) \\
...
$A \in \mathbb{R}^{n\times n}$ & Adjacency matrix ($A_{ij}=1$ if \(v_i\) and \(v_j\) are adjacent, else $0$; in weighted graphs, $A_{ij}$ stores the edge weight, while neighbourhoods are defined by edge incidence unless stated otherwise) \\
...
$\mathcal{N}(v)$ & 1-hop neighbourhood of $v$, i.e.\ the set of vertices adjacent to \(v\) \\
...
$\pi$ & Permutation of $\{1,\dots,n\}$ (vertex relabelling) \\
...
$\hat{C}$ & Re-encoded graph-level vector of a generated graph, computed using the same fitted condition pipeline as \(C\) \\
$d_C$ & Dimension of the graph-level condition vector \\
$\phi_{\mathrm{NSPPK}}^{\mathrm{graph}}$ & NSPPK graph-level feature map used as the default condition encoder \\
$\phi_{\mathrm{NSPPK}}^{\mathrm{node}}$ & NSPPK node-level feature map used to construct node structural rows \\
...
$\hat{X}_0$ & Predicted or reconstructed clean padded node-feature matrix \\
$X_t$ & Noisy node-feature matrix at noise level or diffusion time $t$ \\
$\sigma(t)$ & Noise scale at time $t$ \\
$\sigma_{\min},\sigma_{\max}$ & Minimum and maximum noise levels in the generator schedule \\
...
$Z_t$ & Node-latent matrix at diffusion time \(t\), after the current denoising update \\
$z_i$ & Latent representation of node row $i$ \\
$B_{\mathrm{cont}}$ & Feature mask selecting coordinates trained by the continuous denoising loss \\
...
$\mu$ & Momentum parameter used when updating the provisional edge state during sampling \\
$M_t^{\mathrm{mol}}$ & Deterministic molecular-state matrix used by the chemistry-aware variant \\
$\lambda_{\mathrm{mol}}$ & Weight controlling injection of molecular-state features \\
$\mathcal{P}_{\mathrm{sup}}$ & Set of supervised unordered node pairs used for pairwise edge losses \\
$\mathcal{L}_{\bullet}$ & Loss component whose subscript identifies the supervised or auxiliary objective \\
$\lambda_{\bullet}$ & Non-negative weight applied to the corresponding loss component \\
$\eta_{\mathrm{num}}$ & Small numerical stabiliser used in masked loss denominators \\
