...
Appendix Table~\ref{tab:synthetic-cycle-leg-proposed-hyperparameters}. Each ablation changes one
component where possible and is evaluated using the same \(100\) decoded graphs
per requested cycle size. The exact cycle rate counts graphs with the requested
cycle size, while the stricter metric additionally requires the node labels to
match the required structural roles. The Baseline GCDG row therefore corresponds to
...
No node-count and node-label histogram controls & 0.165 & 0.010 \\
...
No distance supervision, with balanced degree loss & 0.257 & 0.160 \\
...
labels. Two rows change coupled settings: the node-count and node-label
histogram controls are disabled together, and the distance-supervision
ablation also enables balanced degree loss. These rows should therefore be
interpreted as combined sensitivity checks.
}
...
node-count control, node-label supervision, degree supervision, or the
clean-\(X_0\) reconstruction loss mainly damages the strict score, which means
that the generator can still produce cycle-like graphs but loses the alignment
between structure and labels.
