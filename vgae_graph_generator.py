...
class _VGAEModel(nn.Module):
    ...
    def encode(self, x: torch.Tensor, adj_norm: torch.Tensor) -> torch.Tensor:
        ...
        eps = torch.randn(x.size(0), self.hidden2_dim, device=x.device, dtype=x.dtype)
...
class VGAEGraphGenerator:
    ...
    def __init__(
        ...
        *,
        ...
        self._edge_label_values: List[int] = []
        self._edge_label_to_idx: Dict[int, int] = {}
    ...
    @staticmethod
    def inspect_edge_labels(graphs: Sequence[nx.Graph]) -> Dict[str, Any]:
        """Summarize canonical edge labels in a graph collection."""
        ...
        for graph in graphs:
            for _u, _v, data in graph.edges(data=True):
                total += 1
                raw = data.get("label", data.get("edge_attr"))
                if raw is None:
                    missing += 1
                    continue
                counts[int(raw)] += 1
        ...
        return {
            ...
            "is_non_uniform": len(values) > 1,
        }

    def _fit_edge_label_vocabulary(self, graphs: Sequence[nx.Graph]) -> None:
        """
        Fit the edge-label vocabulary used by incident-label node features.

        A single uniform edge label carries no semantic distinction, so no
        additional dimensions are created in that case.
        """
        summary = self.inspect_edge_labels(graphs)
        values = list(summary["unique_labels"]) if summary["is_non_uniform"] else []
        self._edge_label_values = values
        self._edge_label_to_idx = {label: idx for idx, label in enumerate(values)}

    @staticmethod
    def _canonical_graph(graph: nx.Graph) -> nx.Graph:
        """
        Copy a graph into the dense row ordering expected by this VGAE.

        NetworkX graphs can have arbitrary node ids. The VGAE tensors are one
        row per node, so normalizing once at fit time keeps features and
        adjacency rows aligned.
        """
        graph = nx.convert_node_labels_to_integers(graph.copy(), ordering="default")
        for u in graph.nodes():
            graph.nodes[u]["label"] = int(graph.nodes[u].get("label", graph.nodes[u].get("attr", 0)))
        for u, v in graph.edges():
            graph.edges[u, v]["label"] = int(
                graph.edges[u, v].get("label", graph.edges[u, v].get("edge_attr", 0))
            )
        return graph
    ...
    def _build_raw_features(
        ...
        *,
        ...
        Features are node-label one-hot vectors, optional normalized incident
        edge-label histograms, and structural features: normalized degree,
        ...
        n_nodes = len(nodes)
        labels = np.array([int(graph.nodes[u].get("label", 0)) for u in nodes], dtype=np.int64)
        labels = np.clip(labels, 0, n_label_features - 1)

        label_one_hot = np.eye(n_label_features, dtype=np.float32)[labels]
        edge_label_to_idx = getattr(self, "_edge_label_to_idx", {})
        incident_edge_labels = np.zeros(
            (n_nodes, len(edge_label_to_idx)),
            dtype=np.float32,
        )
        if edge_label_to_idx:
            node_to_row = {node: row for row, node in enumerate(nodes)}
            for u, v, data in graph.edges(data=True):
                raw = data.get("label", data.get("edge_attr"))
                if raw is None:
                    continue
                edge_idx = edge_label_to_idx.get(int(raw))
                if edge_idx is None:
                    continue
                incident_edge_labels[node_to_row[u], edge_idx] += 1.0
                incident_edge_labels[node_to_row[v], edge_idx] += 1.0
            row_sums = incident_edge_labels.sum(axis=1, keepdims=True)
            incident_edge_labels = np.divide(
                incident_edge_labels,
                np.maximum(row_sums, 1.0),
                out=np.zeros_like(incident_edge_labels),
                where=row_sums > 0,
            )

        degrees = np.array([graph.degree(u) for u in nodes], dtype=np.float32)
        degree_norm = degrees / max(float(max(n_nodes - 1, 1)), 1.0)
        clustering = np.array([nx.clustering(graph, u) for u in nodes], dtype=np.float32)
        centrality_dict = nx.betweenness_centrality(graph, normalized=True)
        centrality = np.array([float(centrality_dict[u]) for u in nodes], dtype=np.float32)
        structural = np.stack([degree_norm, clustering, centrality], axis=1).astype(np.float32)
        features = np.concatenate(
            [label_one_hot, incident_edge_labels, structural],
            axis=1,
        ).astype(np.float32)
    ...
    def _fit_feature_scaler(
        ...
        *,
        ...
        # Keep node/edge categorical blocks unchanged; scale structural features only.
        edge_feature_dim = len(getattr(self, "_edge_label_to_idx", {}))
        structural_start = int(n_label_features) + edge_feature_dim
        ...
        return {
            ...
            "edge_label_values": list(getattr(self, "_edge_label_values", [])),
    ...
    def fit(
        ...
        graphs = [self._canonical_graph(g) for g in graphs]
        self._label_feature_dim = self._infer_label_feature_dim(graphs)
        self._fit_edge_label_vocabulary(graphs)
        if self.verbose:
            edge_summary = self.inspect_edge_labels(graphs)
            print(
                "[VGAE] edge labels:",
                edge_summary["label_counts"],
                "| encoded incident-label dimensions:",
                len(self._edge_label_values),
            )
    ...
    def encode_graphs(
        ...
        *,
        ...
        """
        Encode graphs into deterministic graph-level VGAE posterior summaries.

        The node posterior means are pooled into one vector per graph. When
        ``include_logstd`` is enabled, the pooled log-standard-deviation vector
        is concatenated to the pooled mean.
        ...
        Args:
            ...
            pooling: One of ``mean``, ``max``, or ``mean_max``.
            include_logstd: Concatenate pooled posterior log standard deviation.

        Returns:
            Dense array with one graph-level latent vector per input graph.
        """
        if not self._is_fitted:
            raise RuntimeError("Call fit(...) before encode_graphs(...).")
        ...
        for graph, graph_type in zip(graphs, graph_types):
            ...
            with torch.no_grad():
                _ = model(pack["features_t"], pack["adj_norm_t"])
                node_mean = model.mean
                node_logstd = model.logstd

                if pooling == "mean":
                    pooled_mean = node_mean.mean(dim=0)
                    pooled_logstd = node_logstd.mean(dim=0)
                elif pooling == "max":
                    pooled_mean = node_mean.max(dim=0).values
                    pooled_logstd = node_logstd.max(dim=0).values
                else:
                    pooled_mean = torch.cat(
                        [node_mean.mean(dim=0), node_mean.max(dim=0).values],
                        dim=0,
                    )
                    pooled_logstd = torch.cat(
                        [node_logstd.mean(dim=0), node_logstd.max(dim=0).values],
                        dim=0,
                    )

                latent = (
                    torch.cat([pooled_mean, pooled_logstd], dim=0)
                    if include_logstd
                    else pooled_mean
                )
            graph_latents.append(latent.detach().cpu().numpy().astype(np.float32))

        if not graph_latents:
            width_multiplier = 2 if pooling == "mean_max" else 1
            if include_logstd:
                width_multiplier *= 2
    ...
    def _sample_adjacency_binary(
        ...
        *,
        ...
        """
        eps = torch.randn(num_nodes, mu.numel(), device=mu.device, dtype=mu.dtype)
        z = mu.unsqueeze(0) + eps * std.unsqueeze(0)
        adjacency_prob = torch.sigmoid(z @ z.t()).detach().cpu().numpy()
        ...
        if self.enforce_connected and num_nodes > 1:
            adjacency_bin = self._enforce_connectivity(
                ...
                upper_prob=upper,
                initial_threshold=threshold_eff,
                triu_idx=triu_idx,
            )
