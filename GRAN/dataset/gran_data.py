...
class GRANData(object):
  ...
  def _get_graph_data(self, G):
    ...
    adj_0 = np.asarray(nx.to_numpy_array(G), dtype=np.float32)
    ...
    adj_3 = np.asarray(nx.to_numpy_array(G, nodelist=node_list_bfs), dtype=np.float32)
    adj_4 = np.asarray(nx.to_numpy_array(G, nodelist=node_list_dfs), dtype=np.float32)
    ...
    adj_5 = np.asarray(nx.to_numpy_array(G, nodelist=node_list), dtype=np.float32)
